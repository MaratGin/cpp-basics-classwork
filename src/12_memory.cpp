 ...
 * A: Автоматическим выделением памяти занимается Компилятор, выделяет её на стеке
 * 2. Чем страшна утечка памяти? Приведите примеры.
 * A: Например, если не устранять утечки памяти, 
 * то в течение работы программы оперативная память полностью переполниться, 
 * что приведет к крашу программы
 * 3. Управляют ли вручную памятью в современном C++?
 * A: Новые стандарты языка позволяют упростить работу с памятью (умные указатели и т. д.),
 * Но с памятью также продолжают работать вручную.
 */