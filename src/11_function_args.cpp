 ...
 * A: Да, с помощью модификатора const
 * 2. Как передать массив в функцию по значению (копию)?
 * A: Можно использовать std::array — это контейнер, 
 * который предоставляет массив фиксированного размера с возможностью передачи по значению
