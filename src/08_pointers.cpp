...
/*
 * Задания:
 * 1. Для чего используется указатели? 
 * A: Чтобы иметь полный контроль за памятью, что позволяет писать очень эффективный код на низком уровне
 * В чем заключается проблема использования переменных в простом виде?
 * А: Ограниченная гибкость по сравнению с использованием указателей
 *    И обычные переменный сложно использовать с динамическими типами данных, где используются ссылки и т. д.
 * 2. Можно ли указателем сослаться на "запрещенный" участок памяти?
 * А: да, можно, но в итоге получим оишбку segfault
 * 3. Реализуйте операцию swap - поменять местами значения переменных.
 * A: Функция change_values()
 */